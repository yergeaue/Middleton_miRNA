...
boxall<- ggplot(FACS_df_mean, aes(x=Treatment, y=RBG_P3)) +
  ...
  geom_point(aes(fill=Strain, color=Strain, shape=Strain), position = position_jitterdodge())+theme_bw()+
...
stat.test.pos<-stat.test.Pop %>% add_xy_position(x="Treatment", group="Strain",fun = "max", dodge = 0.75)
...
#Adding the coordinates of the p-values
stat.test.pos_2<-stat.test.MFI%>% add_xy_position(x="Treatment", group="Strain",fun = "max", dodge = 0.75)
